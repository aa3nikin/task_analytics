...
default_select = ['Образование', 'Рыболовство и рыбоводство', 'Строительство', 'Средняя']
...
st.markdown("#### Среднемесячная номинальная начисленная заработная плата по выбранным отраслям:")
st.dataframe(service.get_data(), use_container_width=True)

fig = service.get_salary_plot(show_infl)
st.plotly_chart(fig, use_container_width=True)

st.markdown("#### Зарплаты по отраслям в 2023 году (сортировка по возрастанию)")
fig = service.get_min_max_salary_plot()
st.plotly_chart(fig, use_container_width=True)

st.markdown(f"#### Реальные зарплаты в ценах {years[0]} и {years[1]} годов")
